...
class SubroutineDefinition:
    ...
    def __init__(self, implementation: Callable[..., Expr], returnType: TealType) -> None:
        super().__init__()
        ...
        for name, param in sig.parameters.items():
            if param.kind not in (Parameter.POSITIONAL_ONLY, Parameter.POSITIONAL_OR_KEYWORD):
                raise TealInputError("Function has a parameter type that is not allowed in a subroutine: parameter {} with type {}".format(name, param.kind))
            
            if param.default != Parameter.empty:
                raise TealInputError("Function has a parameter with a default value, which is not allowed in a subroutine: {}".format(name))
    ...
    def invoke(self, args: List[Expr]) -> "SubroutineCall":
        if len(args) != self.argumentCount():
            raise TealInputError("Incorrect number of arguments for subroutine call. Expected {} arguments, got {}".format(self.argumentCount(), len(args)))
        
        for i, arg in enumerate(args):
            if not isinstance(arg, Expr):
                raise TealInputError("Argument at index {} of subroutine call is not a PyTeal expression: {}".format(i, arg))
...
class SubroutineDeclaration(Expr):
    ...
    def __str__(self):
        return "(SubroutineDeclaration \"{}\" {})".format(self.subroutine.name(), self.body)
...
class SubroutineCall(Expr):
    ...
    def __teal__(self, options: 'CompileOptions'):
        verifyTealVersion(Op.callsub.min_version, options.version, "TEAL version too low to use SubroutineCall expression")
...
def Subroutine(returnType: TealType) -> Callable[[Callable[..., Expr]], Callable[..., Expr]]:

    def SubroutineWithArgs(fnImplementation: Callable[..., Expr]) -> Callable[..., Expr]:
...
def evaluateSubroutine(subroutine: SubroutineDefinition) -> SubroutineDeclaration:
    ...
    if not isinstance(subroutineBody, Expr):
        raise TealInputError("Subroutine function does not return a PyTeal expression. Got type {}".format(type(subroutineBody)))
