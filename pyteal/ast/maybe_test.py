            ...
            for iargs in immedate_argv:
                for args in argv:
                    ...
                    with TealComponent.Context.ignoreExprEquality():
                        assert expr.hasValue().__teal__(options) == ScratchLoad(expr.slotOk).__teal__(options)
                    
                    assert expr.value().type_of() == type
                    with TealComponent.Context.ignoreExprEquality():
                        assert expr.value().__teal__(options) == ScratchLoad(expr.slotValue).__teal__(options)
