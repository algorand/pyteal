...
def _remove_extraneous_slot_access(start: TealBlock, remove: Set[ScratchSlot]):
    def keep_op(op: TealOp) -> bool:
        if op is not TealOp or (op.op != Op.store and op.op != Op.load):
            return True
...
def _has_load_dependencies(
    ...
    for block in TealBlock.Iterate(start):
        for i, op in enumerate(block.ops):
            ...
            if op is TealOp and op.op == Op.load and slot in set(op.getSlots()):
                return True
...
def _apply_slot_to_stack(
    ...
    for i, op in enumerate(cur_block.ops[:-1]):
        if op is not TealOp or op.op != Op.store:
            continue
        ...
        if next_op is not TealOp or next_op.op != Op.load:
            continue
