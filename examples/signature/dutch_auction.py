...
def dutch_auction(start_round = tmpl_start_round,
    ...
    bid = And(Gtxn[0].rekey_to() == Global.zero_address(),
    ...
    redeem = And(Gtxn[0].rekey_to() == Global.zero_address(),
                Gtxn[1].rekey_to() == Global.zero_address(),
                Gtxn[2].rekey_to() == Global.zero_address(),
                Gtxn[3].rekey_to() == Global.zero_address(),
    ...
    wrapup = And(Txn.rekey_to() == Global.zero_address(),
                Txn.first_valid() >= start_round + wrapup_time,
                Or(And(Txn.type_enum() == TxnType.Payment,
                        Txn.amount() == Int(0),
